...
summary_change <- change_draws %>% group_by(name) %>% 
  summarise(medianDiff = median(value),
            lowerLim = quantile(value,0.05), 
            upperLim = quantile(value,0.95))
...
boxplots <- ggplot() + 
  ...
  geom_point(data=mean_persubject_persession[which(grepl("w_final_1|z_final_1",mean_persubject_persession$name)),],
             aes(x=as.factor(LeverHeight),y=log(value),fill=as.factor(LeverHeight)),color="black") + 
  ...
  scale_fill_manual(values=c("black","grey","white"))+ 
  facet_wrap(~name, scales="free", 
             labeller=labeller(.cols=c(w_final_1 = "w",z_final_1 = "z"))) + 
  ggthemes::theme_tufte()+
...
w_change <-   ggplot() + 
  ...
  scale_fill_discrete(name="Condition Change", 
                       labels=c("1->11 cm","11->18 cm"), 
                      type = c("grey","white")) + 
  ggthemes::theme_tufte()+
  theme(axis.text.y = element_blank(), 
        axis.ticks.y = element_blank())+  xlim(-5,2.5) + 
  ylab("Difference Estimate") + 
  xlab("Estimate") + 
  ggtitle("Change in w")
...
z_change_ridgeline <-   ggplot() + 
    geom_density_ridges(data=change_draws[grepl("z",change_draws$name),],
                       aes(x=value,y=name,fill=name), panel_scaling = FALSE) + 
  geom_vline(data=change_draws[grepl("z",change_draws$name),],
             aes(xintercept = 0)) + 
  scale_fill_discrete(name="Condition Change", 
                       labels=c("1->11 cm","11->18 cm"), 
...
patched_plot <- (w_change + z_change) / boxplots + plot_layout(guides="collect")
patched_plot
